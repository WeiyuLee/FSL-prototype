...
from utils import log10, get_batch, get_batch_by_class, img_merge, mkdir_p
...
class model2(object):
    ...
    def __init__(self, batch_size, max_iters, repeat, dropout, model_path, data_ob, log_dir, output_dir, learnrate_init, anoCls,
        ...
        self.model_list = ["AD_DISE", "AD_CLS_DISE"]
...
#        self.cls_acc = tf.reduce_mean(tf.cast(cls_correct_pred, tf.float32))             
        ...
        self.g_loss = 1*self.content_loss + 10*self.code_loss + disc2_fake_loss + (self.adv_weight * disc_fake_loss) + (self.adv_weight * self.dise_loss)
    ...
    def build_eval_AD_CLS_DISE(self):
        ...
        # Encoder
        _, self.z1_con, self.z2 = mz.build_model({"mode":"encoder", "en_input":self.images, "reuse":False})         
        
        # Code
        self.t_code = tf.concat([self.z1_src, self.z1_con, self.z2], -1)
        
        # Decoder        
        self.decoder_output = mz.build_model({"mode":"decoder", "code":self.t_code, "reuse":False})      
        
        # fixed Encoder
        self.f_z1, _, _ = mz.build_model({"mode":"encoder", "en_input":self.decoder_output, "reuse":True})         
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(10):
                ...
                while True:
                    ...
                    decoder_output = sess.run([self.decoder_output], feed_dict=fd_test)               
                    decoder_output = np.squeeze(np.array(decoder_output))
    
                    if output_decode_image == []:
                        output_decode_image = decoder_output
                    else:
                        output_decode_image = np.append(output_decode_image, decoder_output, axis=0)
                ...
                imageio.imwrite(os.path.join(output_path, decode_output_name), output_decode_image)
                imageio.imwrite(os.path.join(output_path, encode_output_name), output_encode_image)
