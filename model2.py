...
class model2(object):
    ...
    def __init__(self, batch_size, max_iters, repeat, dropout, model_path, data_ob, log_dir, output_dir, learnrate_init, anoCls,
        ...
        self.model_list = ["AD_DISE", "AD_CLS_DISE", "AD_CLS_DISE2"]
...
#                imageio.imwrite(os.path.join(output_path, 'Disentanglement', encode_output_name), output_encode_image)
    ...
    def build_AD_CLS_DISE2(self):
        ...
        self.g_loss = 50*self.content_loss + 25*self.code_loss + disc2_fake_loss + (1 * self.adv_weight * self.dise_loss) + (self.adv_weight * disc_fake_loss) 
        self.d_loss = -(disc_fake_loss - disc_ture_loss) + d_gp                               
        self.d2_loss = -(disc2_fake_loss - disc2_ture_loss) + d2_gp                      
    ...
    def train_AD_CLS_DISE2(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            epoch_pbar = tqdm(range(start_step, self.max_iters))
    ...
    def test_AD_CLS_DISE2(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(10):
                ...
                output_decode_image = []
                output_encode_image = []
                output_ano_score = np.empty((0, 21))
                ...
                while True:
                    ...
                    if i == input_class:
                        ...
                        ano_score, z1_softmax, z1_2nd_softmax, t_decoder_output = sess.run([self.anomaly_score, self.z1_softmax, self.z1_2nd_softmax, self.t_decoder_output], feed_dict=fd_ano_score)                   
                        t_decoder_output = np.squeeze(np.array(t_decoder_output))
                        ano_score = np.expand_dims(ano_score, axis=-1) 

                        output_ano_score = np.append(output_ano_score, np.hstack((ano_score, z1_softmax, z1_2nd_softmax)), axis=0)
